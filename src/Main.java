...
//        хранится сумма затрат по дням (а не каждая сумма отдельно). То есть в книге 30 записей разных сумм. Эти
//        суммы разнятся от 100 000 до 200 000.
//
//        Ваш коллега по отделу написал код, который генерирует массив из 30 случайных чисел, и предложил прежде, чем
//        писать программу для настоящей бухгалтерской книги, проверить всё на случайных числах от 100 000 до 200 000.
//
...
//
//<aside>
//💡 **Важно**: метод для генерации массива, который мы предлагаем использовать выше для решения задач, нужно писать
// на одном уровне с методом main. Не внутри мейна, не вне класса с домашним заданием.
//
//                </aside>
//
//```java
...
//
//        Если у вас возникла такая проблема, нужно навести на красное и нажать Alt + Enter или прописать в самом верху
//        файла, над строкой с class, но ниже строки package, следующее:
//
//    ```java
...
//    – Написанный код выводит в консоль корректные данные.
    ...
    // Задача 1..
        ...
        for (int i = 0; i < array.length; i++) {        //внесем даные зарплат в массив
            array[i] = (int) (costs * Math.random() + costs);
            System.out.println(i + ") " + array[i]);
        }
        //Сумма зарплат за месяц
        int sumCosts = 0;
        for (int i = 0; i < array.length; i++) {
            sumCosts = sumCosts + array[i];
        }
        System.out.println("Сумма трат за месяц составила " + sumCosts + "рублей.");

        // Задача 2..
        int minCosts = array[0]; //Максимальные
        int maxCosts = array[0];
        for (int i = 1; i < array.length; i++) {
            if (array[i] >= maxCosts){
                maxCosts = array[i];
            } else if (array[i] < minCosts) {
                minCosts = array[i];
            }
        }
